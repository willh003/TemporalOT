...
class S3D(nn.Module):
    ...
    def _space_to_depth(self, input):
        ...
        # Ensure that T, H, and W are even
        T_pad = (T + 1) // 2 * 2
        H_pad = (H + 1) // 2 * 2
        W_pad = (W + 1) // 2 * 2
        
        if T_pad != T or H_pad != H or W_pad != W:
            # Pad the input if necessary
            pad_t = (0, T_pad - T)
            pad_h = (0, H_pad - H)
            pad_w = (0, W_pad - W)
            input = F.pad(input, pad_w + pad_h + pad_t)
        
        input = input.view(B, C, T_pad // 2, 2, H_pad // 2, 2, W_pad // 2, 2)
        input = input.permute(0, 3, 5, 7, 1, 2, 4, 6)
        input = input.contiguous().view(B, 8 * C, T_pad // 2, H_pad // 2, W_pad // 2)
        return input
